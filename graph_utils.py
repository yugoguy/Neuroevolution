...
def node_depths(genome: Genome) -> dict[int, int]:
    ...
    Inputs/bias (no incoming enabled edge) sit at depth 0; depth is the length of
    the longest enabled path reaching a node. Relies on the feed-forward (acyclic)
    invariant, so a Kahn-style topological sweep terminates.
    """
