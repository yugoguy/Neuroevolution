...
Each operator is an independent function that edits a genome in place (the caller
passes a fresh copy). `mutate` composes them by probability. All rates and
scales are arguments, so they live in config rather than here.

Feed-forward invariant: connections may only be added if they keep the enabled
graph acyclic, enforced by a reachability check.
"""
...
from genome import Genome, NodeGene, ConnGene, HIDDEN, INPUT, OUTPUT, io_ids
...
def mutate_add_node(
    ...
    registry: InnovationRegistry,
    n_max: int,
    new_activation: str = "tanh",
) -> None:
    """Split a random enabled connection, inserting a hidden node.

    The in->new edge gets weight 1 and the new->out edge inherits the old weight,
    so the network's function is unchanged at the moment of mutation. Skipped if
    the genome already holds `n_max` nodes (the converter's tensor budget).
    """
    ...
    c.enabled = False
    genome.node_genes[node_id] = NodeGene(node_id, HIDDEN, new_activation)
...
def mutate(
    ...
    *,
    ...
    if rng.random() < p_add_connection:
        mutate_add_connection(g, rng, registry, weight_init_std)
    if rng.random() < p_add_node:
        mutate_add_node(g, rng, registry, n_max)
