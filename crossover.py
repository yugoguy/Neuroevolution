...
Recombines two parents aligned by innovation number:
  - matching genes (shared innovation): inherited from a random parent;
  - disjoint/excess genes: inherited from the fitter parent only
    (from both parents on a fitness tie).
...
def crossover(
    ...
    # Which innovations the child considers: the fitter parent's, plus the
    # less-fit parent's on a tie.
    ...
    for innov in innovations:
        ...
        if g_fit is not None and g_oth is not None:           # matching
            chosen = replace(g_fit if rng.random() < 0.5 else g_oth)
    ...
    # Child nodes: every node referenced by an inherited connection, plus the
    # reserved nodes (always present). Activations come from the fitter parent
    # when available, else the other.
    node_ids = set()
